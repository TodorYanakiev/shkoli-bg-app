...
type LyceumSubscriptionActions = {
...
const hasValidLyceumId = (lyceumId?: number): lyceumId is number =>
  typeof lyceumId === 'number' && Number.isFinite(lyceumId)

export const useLyceumSubscriptionActions = (
  lyceumId?: number,
): LyceumSubscriptionActions => {
  ...
  useEffect(() => {
    ...
    setActionError(null)
  }, [currentUserId, lyceumId])

  const subscribeMutation = useMutation<void, AppError, void>({
    mutationFn: async () => {
      if (!hasValidLyceumId(lyceumId)) {
        throw createInvalidSubscriptionTargetError('lyceum')
      }

      try {
        await subscribeToLyceum(lyceumId)
      } catch (error) {
        throw mapSubscriptionApiError(error, 'lyceum', 'subscribe')
      }
    },
    retry: false,
  })
  ...
  const unsubscribeMutation = useMutation<void, AppError, void>({
    mutationFn: async () => {
      ...
      try {
        await unsubscribeFromLyceum(lyceumId)
      } catch (error) {
        throw mapSubscriptionApiError(error, 'lyceum', 'unsubscribe')
      }
    },
    retry: false,
  })
  ...
  const syncSubscriptionState = useCallback(
    (nextValue: boolean) => {
      ...
      setStoredSubscriptionState(currentUserId, 'lyceum', lyceumId, nextValue)
    },
    [currentUserId, lyceumId],
  )
  ...
  const onToggleSubscription = useCallback(async () => {
    ...
    try {
      if (isSubscribed) {
        ...
        showToast({
          message: t('feedback.lyceums.unsubscribed'),
          tone: 'success',
        })
        return
      }
      ...
      await refreshCurrentUser()
      showToast({
        message: t('feedback.lyceums.subscribed'),
        tone: 'success',
      })
    } catch (error) {
      const appError = error as AppError

      if (appError.messageKey === 'errors.lyceums.alreadySubscribed') {
        syncSubscriptionState(true)
        showToast({
      ...
      if (appError.messageKey === 'errors.lyceums.notSubscribed') {
        syncSubscriptionState(false)
        showToast({
          message: t(appError.messageKey),
          tone: 'info',
