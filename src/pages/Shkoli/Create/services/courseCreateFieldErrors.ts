...
const directFieldMap: Record<string, Path<CourseCreateFormValues>> = {
  name: 'name',
...
const toScheduleSlotPath = (
...
): Path<CourseCreateFormValues> | null => {
  ...
  return `scheduleSlots.${index}.${fieldName}` as Path<CourseCreateFormValues>
}
...
const toScheduleSpecialCasePath = (
  ...
  return `scheduleSpecialCases.${index}.${fieldName}` as Path<CourseCreateFormValues>
}

const resolveFormFieldPaths = (
  rawField: string,
): Path<CourseCreateFormValues>[] => {
...
type ApplyCourseCreateServerFieldErrorsParams = {
  error: ApiError
  setError: UseFormSetError<CourseCreateFormValues>
  t: TFunction
}

export const applyCourseCreateServerFieldErrors = ({
  error,
  setError,
  t,
}: ApplyCourseCreateServerFieldErrorsParams) => {
  ...
  if (normalizedMessage.includes('schedule')) {
    setError('scheduleSlots', {
      type: 'server',
      message: t('errors.courses.createInvalid'),
    })
    hasMappedFieldError = true
  }
