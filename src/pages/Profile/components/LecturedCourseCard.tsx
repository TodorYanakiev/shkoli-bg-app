...
const LecturedCourseCard = ({
  course,
  lecturerName,
  additionalLecturers,
  fallbackValue,
}: LecturedCourseCardProps) => {
  ...
  const cardClassName =
    'group relative flex h-full w-full max-w-sm flex-col overflow-hidden rounded-2xl border border-slate-200/70 bg-white/90 shadow-sm backdrop-blur transition hover:-translate-y-1 hover:shadow-lg'
  const cardContent = (
    <article className={cardClassName}>
      <div className="relative">
        <img
          ...
          alt={
            mainImage?.altText ??
            t('pages.profile.lecturedCourses.card.imageAlt', {
              name: courseName,
            })
          }
        ...
        <div className="absolute -bottom-4 left-4 rounded-2xl border border-white/80 bg-white/90 p-1 shadow-md">
          <img
            ...
            alt={
              logoImage?.altText ??
              t('pages.profile.lecturedCourses.card.logoAlt', {
                name: courseName,
              })
            }
      ...
      <div className="flex flex-1 flex-col px-4 pb-4 pt-6">
        ...
        <p className="mt-2 text-xs text-slate-600">
          <span className="text-slate-500">
            {t('pages.profile.lecturedCourses.card.lecturerLabel')}
          </span>{' '}
          ...
          {additionalLecturers > 0 ? (
            <span className="text-slate-500">
              {' '}
              {t('pages.profile.lecturedCourses.card.additionalLecturers', {
  ...
  return (
    <Link
      to={courseLink}
      aria-label={t('pages.profile.lecturedCourses.card.openCourse', {
        name: courseName,
      })}
...
export default LecturedCourseCard
