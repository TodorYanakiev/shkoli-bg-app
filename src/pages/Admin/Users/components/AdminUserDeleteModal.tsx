...
export const AdminUserDeleteModal = ({
  isOpen,
  userName,
  onConfirm,
  onCancel,
  ...
  const titleId = 'admin-user-delete-title'
  const descriptionId = 'admin-user-delete-description'
  const resolvedName = userName ?? t('pages.profile.emptyValue')
      ...
      >
        <div className="relative rounded-2xl border border-slate-200/70 bg-white/90 p-5 shadow-sm">
          ...
          <h3 id={titleId} className="text-sm font-semibold text-slate-900">
            {t('pages.admin.users.deleteConfirm.title')}
          </h3>
          <p id={descriptionId} className="mt-2 text-sm text-slate-600">
            {t('pages.admin.users.deleteConfirm.description', {
              name: resolvedName,
            })}
          </p>
          <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {t('pages.admin.users.deleteConfirm.cancel')}
            </button>
            <button
              type="button"
              onClick={onConfirm}
            ...
            >
              {isSubmitting
                ...
                : t('pages.admin.users.deleteConfirm.confirm')}
            </button>
          </div>
        </div>
      </div>
