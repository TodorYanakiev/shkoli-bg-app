...
type AdminLyceumDeleteModalProps = {
  isOpen: boolean
  lyceumName?: string
  onConfirm: () => void
  onCancel: () => void
...
export const AdminLyceumDeleteModal = ({
  isOpen,
  lyceumName,
  onConfirm,
  onCancel,
  isSubmitting = false,
}: AdminLyceumDeleteModalProps) => {
  ...
  const titleId = 'admin-lyceum-delete-title'
  const descriptionId = 'admin-lyceum-delete-description'
  const resolvedName =
    lyceumName ?? t('pages.lyceums.detail.notProvided')
      ...
      >
        <div className="relative rounded-2xl border border-slate-200/70 bg-white/90 p-5 shadow-sm">
          ...
          <h3 id={titleId} className="text-sm font-semibold text-slate-900">
            {t('pages.admin.lyceums.deleteConfirm.title')}
          </h3>
          <p id={descriptionId} className="mt-2 text-sm text-slate-600">
            {t('pages.admin.lyceums.deleteConfirm.description', {
              name: resolvedName,
            })}
          </p>
          <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {t('pages.admin.lyceums.deleteConfirm.cancel')}
            </button>
            <button
              type="button"
              onClick={onConfirm}
            ...
            >
              {isSubmitting
                ...
                : t('pages.admin.lyceums.deleteConfirm.confirm')}
            </button>
          </div>
        </div>
      </div>
