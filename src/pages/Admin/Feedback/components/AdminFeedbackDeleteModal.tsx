...
type AdminFeedbackDeleteModalProps = {
  isOpen: boolean
...
export const AdminFeedbackDeleteModal = ({
  isOpen,
  feedbackTitle,
  onConfirm,
  onCancel,
  isSubmitting = false,
}: AdminFeedbackDeleteModalProps) => {
  ...
  const titleId = 'admin-feedback-delete-title'
  const descriptionId = 'admin-feedback-delete-description'
  const resolvedTitle = feedbackTitle ?? t('pages.profile.emptyValue')
      ...
      >
        <div className="relative rounded-2xl border border-slate-200/70 bg-white/90 p-5 shadow-sm">
          ...
          <h3 id={titleId} className="text-sm font-semibold text-slate-900">
            {t('pages.admin.feedback.deleteConfirm.title')}
          </h3>
          <p id={descriptionId} className="mt-2 text-sm text-slate-600">
            {t('pages.admin.feedback.deleteConfirm.description', {
              title: resolvedTitle,
            })}
          </p>
          <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {t('pages.admin.feedback.deleteConfirm.cancel')}
            </button>
            <button
              type="button"
              onClick={onConfirm}
            ...
            >
              {isSubmitting
                ...
                : t('pages.admin.feedback.deleteConfirm.confirm')}
            </button>
          </div>
        </div>
      </div>
